...
def index():
	...
	posts = current_user.followed_posts().paginate(1, app.config['POSTS_PER_PAGE'], False)
...
def explore():
	page = request.args.get('page', 1, type=int)
	posts = Post.query.order_by(Post.timestamp.desc()).paginate(1, app.config['POSTS_PER_PAGE'], False)
